...
function updateSortIcons() {
  ...
  const activeTh = document.querySelector(
    `th[onclick="handleSort('${currentSort.field}')"]`,
  );
  if (activeTh) {
...
function loadCurrentView() {
  if (currentFilter.type === 'ALL') loadAllOs(false);
    ...
    filterByReason(currentFilter.value, false);
  else if (currentFilter.type === 'CLIENT')
    loadOsByClient(currentFilter.value, false);
  else if (currentFilter.type === 'STATUS')
    loadOsByStatus(false, currentFilter.value);
}
...
function loadOsByMonth(month, year, resetPage = true) {
  ...
  fetchOs(
    `${API_URL}/api/os/getByMonth?month=${month}&year=${year}${getSortParam()}`,
  );
}
...
function loadOsByClient(clientName, resetPage = true) {
  ...
  fetchOs(
    `${API_URL}/api/os/getByClient?client=${encodeURIComponent(clientName)}${getSortParam()}`,
  );
...
function filterByReason(reason, resetPage = true) {
  if (
    currentFilter.type === 'REASON' &&
    currentFilter.value === reason &&
    resetPage
  ) {
    resetFilters();
    return;
  ...
  document
    ...
    .forEach((b) => b.classList.remove('active'));
  document
    .querySelector(`.reason-btn.reason-${reason}`)
...
async function searchClients(query) {
  try {
    const response = await Auth.fetch(
      `${API_URL}/api/os/getByClient?client=${query}&size=5`,
    );
    const data = await response.json();
...
function renderOsList(osList) {
  ...
  if (!osList || osList.length === 0) {
    listContainer.innerHTML =
      '<tr><td colspan="5" style="text-align: center; color: var(--text-muted);">Nenhuma OS encontrada.</td></tr>';
    return;
  }
  ...
  osList.forEach((os) => {
    ...
    row.innerHTML = `
            ...
            <td>
                <span class="badge-reason" style="background: ${REASON_COLORS[os.reason] || '#ccc'};">${reason}</span>
            </td>
            <td>${date}</td>
            <td>${statusSelect}</td>
            <td>
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (currentFilter.type === 'ALL') loadAllOs();
        ...
        filterByReason(currentFilter.value);
      else if (currentFilter.type === 'CLIENT')
        loadOsByClient(currentFilter.value);
      else if (currentFilter.type === 'STATUS')
        loadOsByStatus(true, currentFilter.value);
    } else {
      const data = await response.json();
      showToast(
        'Erro ao deletar: ' + (data.message || 'Permissão negada'),
        'error',
      );
    }
  } catch (e) {
...
function openCreateModal() {
  ...
  document
    .getElementById('newClientName')
    .classList.remove('input-error', 'shake');
  document.getElementById('clientError').style.display = 'none';
...
function selectReason(btn, value) {
  document
    .querySelectorAll('#createModal .reason-btn')
    .forEach((b) => b.classList.remove('active'));
...
async function handleCreateOs(e) {
  ...
  const url = isEdit
    ? `${API_URL}/api/os/update/${osId}`
    : `${API_URL}/api/os/createOs`;
  ...
  try {
    ...
    if (response.ok) {
      showToast(
        isEdit ? 'OS atualizada com sucesso!' : 'OS criada com sucesso!',
        'success',
      );
      closeCreateModal();
      loadCurrentView();
...
function parseCSV(text) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (REASONS[reasonUpper]) reason = reasonUpper;
      ...
      const match = Object.keys(REASONS).find(
        (k) => k.includes(reasonUpper) || reasonUpper.includes(k),
      );
    ...
    if (sL.includes('concluido') || sL.includes('concluído'))
...
window.executeImport = async function () {
  ...
  showToast(
    `Importação finalizada! Sucesso: ${successCount}, Erros: ${errorCount}`,
...
async function generateTestOs(count = 20) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const randomReason =
      reasonKeys[Math.floor(Math.random() * reasonKeys.length)];
...
async function exportCsv() {
  ...
  try {
    ...
    fullList.forEach((os) => {
      ...
      const row = [`"${os.client}"`, `"${reason}"`, date, statusLabel];
...
window.deleteAllOs = async function () {
  if (
    !confirm(
      'ATENÇÃO: Isso apagará todas as suas Ordens de Serviço permanentemente. Tem certeza?',
    )
  )
    return;
