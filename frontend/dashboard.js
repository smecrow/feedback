const Dashboard = {
  ...
  fetchStats: async function (queryString = '') {
    ...
    try {
      const url = queryString
        ? `${API_URL}/api/dashboard/stats?${queryString}`
  ...
  setLoading: function (isLoading) {
    ...
    containers.forEach((id) => {
      ...
      if (!parent.classList.contains('chart-container-relative'))
        parent.classList.add('chart-container-relative');
      if (isLoading) {
    ...
    if (isLoading) {
      tbody.innerHTML =
        '<tr><td colspan="4" style="text-align:center; padding: 2rem;"><div class="spinner" style="margin:0 auto; width: 24px; height: 24px;"></div></td></tr>';
    }
  },
  ...
  exportData: function (metric) {
    ...
    if (metric === 'months') {
      const months = this.data.totalByMonths || {};
      csvContent = 'Mes,Quantidade\n';
      Object.keys(months).forEach((m) => {
        csvContent += `${m},${months[m]}\n`;
      });
      ...
      Object.keys(statuses).forEach((s) => {
        const label =
          s === 'OS_REALIZADA'
            ...
            : s === 'FEEDBACK_ENVIADO'
              ? 'Feedback Enviado'
              : s === 'FEEDBACK_CONCLUIDO'
                ? 'Feedback Concluido'
                : 'Pendente';
        csvContent += `${label},${statuses[s]}\n`;
      });
    } else if (metric === 'clients') {
      const clients = this.data.topClients || {};
      csvContent = 'Cliente,Chamados\n';
      Object.keys(clients).forEach((c) => {
        csvContent += `${c},${clients[c]}\n`;
      });
    } else if (metric === 'reasons') {
      const reasons = this.data.totalByReasons || {};
      csvContent = 'Motivo,Quantidade\n';
      Object.keys(reasons).forEach((r) => {
        csvContent += `${this.formatReason(r)},${reasons[r]}\n`;
      });
    }
  ...
  renderKPIs: function (data) {
    ...
    if (greetingContainer) {
      ...
      if (greetingPending)
        greetingPending.innerText =
          absolutePending === 1
            ? 'Você tem 1 feedback pendente.'
            : `Você tem ${absolutePending} feedbacks pendentes.`;
    }
    ...
    const lastMonthName = date
      .toLocaleString('en-US', { month: 'long' })
      .toUpperCase();

    const currentMonthCount =
      (data.totalByMonths && data.totalByMonths[currentMonthName]) || 0;
    const lastMonthCount =
      (data.totalByMonths && data.totalByMonths[lastMonthName]) || 0;
    ...
    const kpis = [
      ...
      },
      {
        ...
        value: pending,
        icon: '⏳',
        trendVal: data.totalNotDone_trend,
      },
      {
        id: 'kpiMonth',
        label: 'Este Mês',
  ...
  getTrendHtml: function (val) {
    if (val === undefined || val === null)
      return '<div class="trend-indicator trend-neutral">Sem dados prévios</div>';

    const rounded = Math.round(val);
    if (rounded === 0)
      return '<div class="trend-indicator trend-neutral">― 0%</div>';
  ...
  renderLatestTable: function (list) {
    ...
    if (!list || list.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="4" style="text-align:center; color: var(--text-muted);">Nenhuma atividade recente.</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = limited
      .map((item) => {
        ...
        if (item.status === 'OS_REALIZADA') {
          statusLabel = 'OS REALIZADA';
          badgeClass = 'os_realizada';
        }
        if (item.status === 'FEEDBACK_ENVIADO') {
          statusLabel = 'FEEDBACK ENVIADO';
          badgeClass = 'feedback_enviado';
        }
        if (item.status === 'FEEDBACK_CONCLUIDO') {
          statusLabel = 'FEEDBACK CONCLUÍDO';
          badgeClass = 'feedback_concluido';
        }

        const statusBadge = `<div class="badge-status ${badgeClass}">${statusLabel}</div>`;

        const formattedReason = this.formatReason(item.reason);
        const reasonColor = this.getReasonColor(item.reason);
        const reasonBadge = `<span class="badge-reason" style="background: ${reasonColor};">${formattedReason}</span>`;
  ...
  renderCharts: function (data) {
    ...
    const statusItems = [
      ...
      {
        key: 'FEEDBACK_CONCLUIDO',
        label: 'Feedback Concluído',
        color: '#10B981',
      },
    ]
    ...
    if (reasonEntries.length === 0) {
      ...
      const detailedLabels = reasonKeys.map((key, index) => {
        ...
        const pct =
          totalReasons > 0 ? Math.round((val / totalReasons) * 100) : 0;
  ...
  formatReason: function (reason) {
    ...
    const map = {
      ...
      N2: 'Suporte N2',
      SINAL_ALTO: 'Sinal Alto',
      CABEAMENTO: 'Cabeamento',
      TV_BOX: 'TV Box',
      EQUIPAMENTO_COM_DEFEITO: 'Equipamento com Defeito',
      RADIO: 'Rádio',
      OUTROS: 'Outros',
    };
  ...
  getReasonColor: function (reason) {
    const map = {
      ...
      CABEAMENTO: '#10b981',
      TV_BOX: '#f43f5e',
      EQUIPAMENTO_COM_DEFEITO: '#d946ef',
      RADIO: '#84cc16',
      OUTROS: '#eab308',
    };
